...
- **Secrets written to disk in plaintext.** The default backend is the OS keychain (macOS Keychain, Windows Credential Manager, Linux Secret Service). File-based storage uses `EncryptedFileStore` (Fernet AES-128-CBC encryption with PBKDF2 key derivation).
